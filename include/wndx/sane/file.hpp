...
#include "path.hpp"    // IWYU pragma: keep
...
struct Finfo
{
  ...
  char        m_fname[fname_max_len]{ "sane_default_file_name\0" };
};
...
template <>
struct fmt::formatter<wndx::sane::file::Finfo> : formatter<string_view>
{
  auto format(wndx::sane::file::Finfo const& f, format_context& ctx) const
      -> format_context::iterator;
};

/// \brief fmt formatter specialization for File.
template <>
struct fmt::formatter<wndx::sane::file::File> : formatter<string_view>
{
  auto format(wndx::sane::file::File const& f, format_context& ctx) const
      -> format_context::iterator;
};
