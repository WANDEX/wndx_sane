    ...
    if (!fs::exists(npath, ec)) { // expected branch
      if (fs::create_directory(npath, parent_dir, ec)) {
        ...
        WNDX_LOG(LL::CRIT,
                 ...
                 fn, npath, npath, ec.value(), ec.message());
...
auto fmt::formatter<wndx::sane::file::Finfo>::format(
    wndx::sane::file::Finfo const& f, format_context& ctx) const
...
// NOLINTNEXTLINE(readability-convert-member-functions-to-static)
auto fmt::formatter<wndx::sane::file::File>::format(
    wndx::sane::file::File const& f, format_context& ctx) const
    -> format_context::iterator
{
...
std::ostream& operator<<(std::ostream& os, wndx::sane::file::Finfo const& f)
{
  return os << fmt::to_string(f);
}

std::ostream& operator<<(std::ostream& os, wndx::sane::file::File const& f)
{
  return os << fmt::to_string(f);
}
